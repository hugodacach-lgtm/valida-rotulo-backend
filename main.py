...
# ══════════════════════════════════════════════════════════════════════════════
# SEED BRIDGE — mapeia chaves MAPA_URLS → chaves dos seeds Supabase
# Permite que get_kb_for_categories acesse o conteúdo dos seeds do KB
# quando o PDF do gov.br não está disponível ou truncado.
# ══════════════════════════════════════════════════════════════════════════════
_SEED_BRIDGE: dict[str, list[str]] = {
    ...
    "presunto":          ["in_portarias_presunto_apresuntado", "in_22_2000_copa_presunto_cru_parma"],
    ...
    "frango_inteiro":    ["nomenclatura_carneos_dipoa_aves_bovinos"],
    ...
    "queijo_coalho_manteiga":["portaria_146_1996_laticinios_gerais"],
    ...
    "queijo_brie_camembert":["portaria_146_1996_laticinios_gerais"],
    ...
    # Bebidas
    "cerveja":        ["in_55_2002_rotulagem_bebidas_vinhos_piq"],
    "cerveja_in14":   ["in_55_2002_rotulagem_bebidas_vinhos_piq"],
    "vinho":          ["lei_7678_1988_decreto_8198_2014_vinhos_derivados", "piq_vinho_espumante_frisante_suco_uva_nao_alcoolicos"],
    "vinho_lei7678":  ["lei_7678_1988_decreto_8198_2014_vinhos_derivados"],
    "cachaça_destilados":["piq_cachaca_destilados_rum_vodca_gin_uisque"],
    ...
    "nomenclatura":      ["nomenclatura_carneos_dipoa_aves_bovinos"],
}
...
async def get_kb_for_categories(categories: list[str]) -> str:
    ...
    Fonte primária: _kb_cache (carregado do Supabase no startup).
    Fonte secundária: seeds do Supabase via _SEED_BRIDGE.
    Fallback: PDF do gov.br (pode falhar por timeout/indisponibilidade).
    """
    if not categories:
        return ""
    ...
    async def load_one(cat: str) -> str:
        # 1ª tentativa: cache direto (categoria == chave do MAPA_URLS)
        if cat in _kb_cache and _kb_cache[cat]:
            return _kb_cache[cat]

        # 2ª tentativa: seeds do Supabase via bridge (chaves do KB seed)
        seed_keys = _SEED_BRIDGE.get(cat, [])
        seed_parts = []
        for sk in seed_keys:
            if sk in _kb_cache and _kb_cache[sk]:
                seed_parts.append(_kb_cache[sk])
        if seed_parts:
            result = "\n\n".join(seed_parts)
            _kb_cache[cat] = result  # cacheia para próximas chamadas
            return result

        # 3ª tentativa: fetch PDF em tempo real (fallback)
...
async def stream_validation(image_b64: str, mime_type: str, obs: str, orgao: str = "",
    ...
    if kb_text:
        ...
        kb_section = f"""## LEGISLAÇÃO ESPECÍFICA — {cats_str}
Textos extraídos da base de conhecimento (KB Supabase + PDFs MAPA) para este produto.
Use como referência primária — cite norma + artigo em cada ponto da validação:
            ...
            async with resp as client:
                r = await client.post(
                    ...
                    headers={"x-api-key": os.environ.get("ANTHROPIC_API_KEY",""),
                             "anthropic-version": "2023-06-01",
                             "content-type": "application/json"},
                    json={"model": "claude-sonnet-4-5-20251022",
                          "max_tokens": 2000,
                          "system": sp_ativo,
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            r = await client.post(
                ...
                headers={"x-api-key": ANTHROPIC_API_KEY,
                         "anthropic-version": "2023-06-01",
                         "content-type": "application/json"},
                json={"model": "claude-sonnet-4-5-20251022", "max_tokens": 2000,
